...
rm $TEST/$task/$config/dummy -r
mkdir -p $TEST/$task/$config/dummy

echo "Predict test cases"

if [ $config == "3d_cascade_fullres" ];
then
	nnUNet_predict -i $nnUNet_raw_data_base/nnUNet_raw_data/Task$task/imagesTs -o $TEST/$task/$config/dummy -t $task -m $config -ctr $trainer -f 0 --disable_tta
else
        nnUNet_predict -i $nnUNet_raw_data_base/nnUNet_raw_data/Task$task/imagesTs -o $TEST/$task/$config/dummy -t $task -m $config -tr $trainer -f 0 --disable_tta
fi


nnUNet_evaluate_folder -ref $nnUNet_raw_data_base/nnUNet_raw_data/Task$task/labelsTs -pred $TEST/$task/$config/dummy -l 1

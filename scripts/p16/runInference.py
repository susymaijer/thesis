...
if __name__ == "__main__":
    ...
    # Get cpu amount
    print(f"How many cpu's do you want to use?")
    cpu=input()
    
    # Define the paths to the folder containing the niftis which we are going to predict
    batch_dir=os.path.join(shared.p16_dir, f'batch{batch}')
    niftis_dir=os.path.join(batch_dir, 'niftis')

    # Get slurm job file path
    ts=datetime.now().strftime("%Y%m%d%H%M%S")
    job_file=os.path.join(shared.job_dir, f"p16_inference_model{task}_batch{batch}_{ts}.job")
    ...
    with open(job_file, "w+") as fh:
        ...
        fh.writelines(f"#SBATCH --error={shared.log_dir}/job.%J.inference.err\n")
        fh.writelines(f"#SBATCH --output={shared.log_dir}/job.%J.inference.out\n\n")

...
job_file="${job_directory}/inference_eval_${task}_${taskPredict}_${p}_${cpu}_${config}_$(date +"%Y_%m_%d_%I_%M_%p").job"

echo "#!/bin/bash
#SBATCH -J ExpPancreasInferenceEval
#SBATCH -p $p
#SBATCH -N 1
#SBATCH --ntasks=1
...
mkdir -p $OUTPUT/$task/$config/$taskPredict/imagesTr
mkdir -p $OUTPUT/$task/$config/$taskPredict/imagesTs

nnUNet_predict -i $nnUNet_raw_data_base/nnUNet_raw_data/Task$taskPredict/imagesTr -o $OUTPUT/$task/$config/$taskPredict/imagesTr -t $task -m $config
nnUNet_predict -i $nnUNet_raw_data_base/nnUNet_raw_data/Task$taskPredict/imagesTs -o $OUTPUT/$task/$config/$taskPredict/imagesTs -t $task -m $config

nnUNet_evaluate_folder -ref $nnUNet_raw_data_base/nnUNet_raw_data/Task$taskPredict/labelsTr -pred $OUTPUT/$task/$config/$taskPredict/imagesTr -l 1
nnUNet_evaluate_folder -ref $nnUNet_raw_data_base/nnUNet_raw_data/Task$taskPredict/labelsTs -pred $OUTPUT/$task/$config/$taskPredict/imagesTs -l 1
